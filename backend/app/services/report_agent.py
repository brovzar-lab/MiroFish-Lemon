...
class ReportAgent:
    ...
    def generate_report(
        ...
        # 检查是否是断点续传（大纲文件已存在）
        is_resume = False
        existing_outline_path = os.path.join(ReportManager._get_report_folder(report_id), "outline.json")
        if os.path.exists(existing_outline_path):
            try:
                with open(existing_outline_path, 'r', encoding='utf-8') as f:
                    outline_data = json.load(f)
                sections = [ReportSection(title=s['title'], content=s.get('content', '')) for s in outline_data.get('sections', [])]
                outline = ReportOutline(
                ...
                is_resume = True
                logger.info(f"断点续传模式：从现有大纲恢复生成 {report_id}")
            except Exception as e:
                logger.warning(f"加载居存大纲失败，重新生成: {e}")
                is_resume = False
        ...
        try:
            if not is_resume:
                ...
                self.report_logger = ReportLogger(report_id)
                self.console_logger = ReportConsoleLogger(report_id)
                logger.info(f"断点续传：已加载大纲，共{len(outline.sections)}个章节")
            ...
            for i, section in enumerate(outline.sections):
                ...
                if is_resume:
                    ...
                    if os.path.exists(section_path):
                        logger.info(f"跳过已生成章节 {section_num}: {section.title}")
                        ...
                        if lines and lines[0].startswith('## '):
                            ...
                            section.content = existing_md
                        generated_sections.append(existing_md)
                        completed_section_titles.append(section.title)
                        ReportManager.update_progress(
                            report_id, "generating",
                            base_progress + int(70 / total_sections),
                            f"章节 {section.title} 已完成（剪载缓存）",
            ...
            # API credits ran out mid-generation — save as PAUSED so Resume can pick up
            logger.error(f"报告生成暂停（API credits 耗尽）: {str(e)}")
            report.status = ReportStatus.FAILED
            paused_msg = (
                "API credits exhausted. "
                f"{len(completed_section_titles)}/{len(getattr(report.outline, 'sections', []))} "
                "sections saved. Top up your OpenRouter balance and click Resume to continue."
            )
            report.error = paused_msg
