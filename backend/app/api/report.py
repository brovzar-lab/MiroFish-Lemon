        ...
        with graph_lifecycle_lock(graph_id):
            ...
            if not force_regenerate:
                ...
                if (
                    ...
                    # Lemon: 报告存在但未完成（失败或生成中），复用同一个
                    # report_id 以便断点续传（Resume 按钮依赖此行为）
                    report_id = existing_report.report_id
            ...
            # Lemon: the report must come out in the project's language
            # (set at launch from the sim-prep package). Request locale is
            # only the fallback.
            current_locale = (
                getattr(refreshed_project, 'language', None)
